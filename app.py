...
# 固定的初始prompt
FIXED_INITIAL_PROMPT = "接下来，我会给你非常复杂的prompt，你需要经过深度思考，拆分成你所认为需要的步骤，接下来，你需要返还你所认为的步骤的量个循序渐进的prompt，作为回答的第一行，你的回答第一行必须是总步骤的数字，不可以有任何额外文字，这些prompt必须是普通的大语言模型可以实现的，简单易懂的prompt，我会给按照顺序吧每个prompt给下一个AI，最后，经过你所认为的数量个prompt，我希望得到一个我刚开始发你的prompt所需要达到的要求和效果。你不可以使用markdown，每一个prompt需要时一段话，可以比较长，中间使用一个空行隔开。拆分的每个prompt必须超过100字。每次换行出现必须代表下一个prompt，不能随便换行。你的回答第一行是一个数字，第二行开始开始就必须是第一个prompt，不可以有额外内容。收到这些prompt的全部是AI，所以很多电脑的工具无法使用，请你确保你给出的方案都是AI可以用的。确保你最后的结果是用户想要的，比如代码，网站，结论等。不要使用：或者类似符号，直接输出1. 2. 3.作为prompt。返还的第一行必须是一个数字不能包含其他内容，这个数字必须是你给出的需要的次数。"
        ...
        if user_prompt:
            ...
            with st.spinner(f"正在获取{st.session_state.selected_model}的响应..."):
                ...
                if response:
                    ...
                    # 添加一个额外的总结步骤
                    prompts.append("请根据之前所有AI的输出，总结并给出最终的完整答复。你的回答应该是对整个任务的最终解决方案。")
